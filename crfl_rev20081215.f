...
c     version (fuchs and muellnd vectorized by
c     k.-j. sandmeier
...
c    Had to rename 2 for the subroutines so that the code would compile
c    under Linux using g77.  The two subroutines zlog() and matmul() were
c    the same as two intrinstic functions in the complier.
c    Renamed them to zlsi() and matmsi().
c    Simon Topping 4/8/99.
...
c iss(22) =1 => use increased precision format for source and time increment
c ( Clare Enright 94 )
      if ( iss(22).eq.1 ) write(6,789)
 789  format( ' high precision source (f14.2) and time increment (f10.8)
     &')
      npa1=npa
      nfr1=nfr
      nla1=nla
...
c
 ...
 104     if(iss(12).eq.1) zs(i)=6370.*(1.-exp(-zs(i)/6370.))
         dso=zs(1)
         nso=1
         nre=1
         do 105 i=1,nlay
...
c     digitized source signal read in from cards
         call signa5(iins,t,dt,s,tsigma,iss(22))
      end if
      do 600 i=1,npts
 600  s(i)=s(i)/fnpt
c
...
c
      call matmsi(rppd2r,rppd2i,rspd2r,rspd2i,rpsd2r,rpsd2i,rssd2r,
     ...
     &d12r,d12i,d21r,d21i,d22r,d22i,nfreq)
      call matmsi(tppu1r,tppu1i,tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,
...
c
     ...
     &a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,nfreq)
      call matmsi(tppu2r,tppu2i,tspu2r,tspu2i,tpsu2r,tpsu2i,tssu2r,
...
c
 1010 call matmsi(tppu1r,tppu1i,tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,
     &tssu1i,rppd2r,rppd2i,rspd2r,rspd2i,rpsd2r,rpsd2i,rssd2r,rssd2i,
     &a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,nfreq)
      call matmsi(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,a11r,a11i,
...
c
 1100 if(ire.ne.0.and.iss(21).eq.1) then
     ...
     &   b22r,b22i,b21r,b21i,nfreq)
         call matmsi(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,c11r,c11i,
...
c
      call matmsi(rppd1r,rppd1i,rspd1r,rspd1i,rpsd1r,rpsd1i,rssd1r,
...
c
      call matmsi(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,tppu1r,tppu1i,
     &tspu1r,tspu1i,tpsu1r,tpsu1i,tssu1r,tssu1i,d11r,d11i,d12r,d12i,
     &d21r,d21i,d22r,d22i,nfreq)
      call matmsi(d11r,d11i,d12r,d12i,d21r,d21i,d22r,d22i,c11r,c11i,c12r
...
c
     ...
     &tppu2i,tspu2r,tspu2i,tssu2r,tssu2i,tpsu2r,tpsu2i,0)
      call matmsi(rppu1r,rppu1i,rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,
...
c
 1310 call matmsi(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,rppu1r,rppu1i,
...
c
      call matmsi(c11r,c11i,c12r,c12i,c21r,c21i,c22r,c22i,a11r,a11i,
     &a12r,a12i,a21r,a21i,a22r,a22i,y11r,y11i,y12r,y12i,y21r,y21i,y22r,
     &y22i,nfreq)
c
...
c                            - x1
c
      call matmsi(y11r,y11i,y12r,y12i,y21r,y21i,y22r,y22i,rppd2r,rppd2i,
...
c
 1320 call matmsi(ub1r,ub1i,ub2r,ub2i,ub3r,ub3i,ub4r,ub4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
c
...
c                            - y1
c
      call matmsi(x11r,x11i,x12r,x12i,x21r,x21i,x22r,x22i,rppd2r,rppd2i,
...
c
      call matmsi(ub1r,ub1i,ub2r,ub2i,ub3r,ub3i,ub4r,ub4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
      call matadd(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,b11r,b11i,b12r
     &,b12i,b21r,b21i,b22r,b22i,c11r,c11i,c12r,c12i,c21r,c21i,c22r,c22i,
...
c
      call matmsi(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,a11r,a11i,a12r
...
c
 1340 call matmsi(ua1r,ua1i,ua2r,ua2i,ua3r,ua3i,ua4r,ua4i,rppu1r,rppu1i,
     &rspu1r,rspu1i,rpsu1r,rpsu1i,rssu1r,rssu1i,b11r,b11i,b12r,b12i,
     &b21r,b21i,b22r,b22i,nfreq)
      if(iss(16).eq.1.and.mdeck.ge.ire) then
         call matmsi(b11r,b11i,b12r,b12i,b21r,b21i,b22r,b22i,a11r,a11i,
...
330      x4i(i)=tpsdi*z1r(i)+tpsdr*z1i(i)
      ...
      call matmsi(x1r,x1i,x2r,x2i,x4r,x4i,x3r,x3i,tppr,tppi,tspr,tspi,
...
340   continue
300   continue
      call matmsi(xppr,xppi,xspr,xspi,xpsr,xpsi,xssr,xssi,x11r,x11i,x12r
...
216   continue
      ...
      x4r(i)=rssur
 211  x4i(i)=rssui
      call matmsi(x1r,x1i,x2r,x2i,x3r,x3i,x4r,x4i,rppr,rppi,rspr,rspi,
     ...
     &x3i,x4r,x4i,nfreq)
      ...
      w4r(i)=tssdr
 212  w4i(i)=tssdi
      call matmsi(x1r,x1i,x2r,x2i,x3r,x3i,x4r,x4i,w1r,w1i,w2r,w2i,w3r,
     &w3i,w4r,w4i,z1r,z1i,z2r,z2i,z11r,z11i,z22r,z22i,nfreq)
      call matmsi(tppr,tppi,tspr,tspi,tpsr,tpsi,tssr,tssi,z1r,z1i,z2r,
...
c
 ...
 100    h22i(i)=aimag(l22)
        ...
        subroutine matmsi(a11r,a11i,a12r,a12i,a21r,a21i,a22r,a22i,
...
c
 ...
 70      format(/1x,'check number of layers')
  ...
  170 format(2f10.3,f10.8,5x,'v-red - initial time - increment')
      return
      end
...
c
 ...
 301  format(10f10.4)
      ...
      subroutine signa5(iins,t,dt,s,tsigma,iss22)
c
c     signa5 - digitized source signal, read in from cards
c
c      common/blo2/nfreq,iss(25),omegai,omegar,omegac(nfr),
c     &phasc(nfr),sphasc(nfr),cphasc(nfr)
      ...
      do 200 j=1,119,8
      jdc=j+7
c allow for increased precision in source (Clare Enright Dec 94)
c      if ( iss(25) .eq. 0 ) then 
c error found WIlma Allan, 15/04/95
c iss(22) controls the source NOT iss(25)
c also, common block edited out as not now  used
c and caused compilation problems
c
      if(iss22.eq.0)then
        read(5,100) (x(i),i=1,8)
      else 
        read(5,120) (x(i),i=1,8)
      end if
      do 210 i=j,jdc
      t1=float(i-1)*dt
      ijk=i-j+1
 ...
 220  write(6,110)
c as above 
c Wilma Allan, 15/04/95
c      if ( iss(25) .eq. 0 ) then 
      if(iss22.eq.0)then
        write(6,100) (s(j),j=1,i)
      else 
        write(6,120) (s(j),j=1,i)
      endif
...
c
 ...
 4740 iballa(i) = 10000.*balla(i)
      abal = tmin - na*dt + x(ke)/vred
      ikom=3
C      write(ioutsh,5001)x(ke),abal,ikom,npts,balmax
 5001 format(2f15.5,i5/,i10,5x,e15.4)
      write(6,4005) x(ke),abal,kom,npts,balmax
 4005 format(2f10.4,i5,i10,5x,e15.4)
      do i=1,npts
      write(ioutsh,*) dt*(i-1), ((iballa(i)/10000.-5.001)/4.99)*balmax
      end do
